...
const setupTextHover = (container, type) => {
    ...
    const animateLetter = (letter, weight, duration = 0.4) => {
        gsap.to(letter, { duration, ease: "power2.out", fontVariationSettings: `'wght' ${weight}` });
    };
...
const Welcome = () => {
  ...
  return (
    <section id='welcome'>
        <p ref={subtitleRef}>{renderText("Hey, I'm Mani Bharathi! Welcome to my", "text-3xl font-georama")}</p>
        <h1 ref={titleRef} className='mt-7'>{renderText("portfolio", "text-9xl italic font-georama")}</h1>

        <div className="small-screen">
            <p>This Portfolio is designed for Desktops/Tablets screens only.</p>
        </div>
    </section>
  )
}
